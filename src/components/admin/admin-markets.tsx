...
export function AdminMarkets() {
  ...
  return <div className="stack-lg admin-operations"><Intro title="Markets" description="Market availability, current round timings and future schedules." />{tx.result && <Alert tone="success" title="Market updated">{tx.result.market.name}</Alert>}<ReadState api={api} empty={!api.data?.markets.length}><ul className="admin-records">{api.data?.markets.map(market => <li key={market.id}><GlassPanel className="stack"><div className="between"><h2 className="type-section-title">{market.name}</h2><Status value={market.state} /></div><p className="type-caption text-secondary">{market.code} · {market.slug} · {market.enabled ? "Enabled" : "Disabled"}</p><RoundFacts market={market} />
