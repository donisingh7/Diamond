...
export function AdminOverview() {
  ...
  const data = api.data;
  return <div className="stack-lg admin-operations"><Intro title="Dashboard" description="Current operations, outstanding requests and today’s activity."><Button variant="secondary" onClick={() => void api.refresh()}>Refresh summary</Button></Intro><ReadState api={api}>{data && <>
    ...
    <GlassPanel className="stack"><div className="between"><h2 className="type-section-title">Recent admin activity</h2><Link className="text-link" href="/admin/audit">Open audit log →</Link></div>{data.recentActivity.length ? <AuditRows logs={data.recentActivity} /> : <p className="text-secondary">No recent activity.</p>}</GlassPanel><p className="type-caption text-muted">Updated <BetTime value={data.serverNow} /></p>
  </>}</ReadState></div>;
}
